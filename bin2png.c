...
int main() {
    ...
    DIR *dir = opendir("for-my-research/viruses");
    ...
    struct stat st = {0};
    if (stat("for-my-research/virusesPNGGreyscale", &st) == -1) {
        mkdir("for-my-research/virusesPNGGreyscale", 0700);
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        char filePath[1024];
        snprintf(filePath, sizeof(filePath), "for-my-research/viruses/%s", entry->d_name);

        if (stat(filePath, &fileStat) == 0 && S_ISREG(fileStat.st_mode)) {
            char outputFile[1024];
            snprintf(outputFile, sizeof(outputFile), "for-my-research/virusesPNGGreyscale/%s.png", entry->d_name);
...
void *binaryToPng(void *args) {
    ...
    // Get final image size (fileSize because each pixel will use 1 byte)
    imageSize = (unsigned int)ceil(sqrt((double) fileSize));
    ...
    x = 0;
    y = 0;
    // Process each byte, add pixel to pngData array
    for(i = 0; i < fileSize; i++) {
        // Set pixel data (greyscale)
        pngData[4 * imageSize * y + 4 * x + 0] = fileBuff[i]; // R
        pngData[4 * imageSize * y + 4 * x + 1] = fileBuff[i]; // G
        pngData[4 * imageSize * y + 4 * x + 2] = fileBuff[i]; // B
    ...
    // Complete the image with black pixels if necessary
    while(x < imageSize && y < imageSize) {
        pngData[4 * imageSize * y + 4 * x + 0] = 0;   // R
